...
- **Server-side OCR Inference**: ONNX Runtime을 활용하여 서버 측에서 PaddleOCR(PP-OCRv4) 모델을 실행합니다.
...
- **Asynchronous Processing**: OCR 요청을 수신하면 작업 ID와 함께 Accepted(202)를 반환하며, 실제 추론과 파싱은 백그라운드 스레드에서 비동기로 처리합니다.
...
- **Infrastructure Layer**: 외부 라이브러리(ONNX, Firebase) 설정 및 데이터베이스 접근 구현체

### Package Structure
```text
com.example.yakallim
├── global                 # 공통 예외 처리 및 유틸리티
...
└── ocr                    # OCR 추론 및 분석
    ├── application        # 비동기 작업 스케줄링 및 태스크 실행
    ├── domain             # OCR 엔진 인터페이스 및 도메인 모델
    ├── infrastructure     # ONNX 엔진, 기울기 보정, 가이드 파서 구현
...
2. `src/main/resources/models/` 경로에 아래 모델 파일과 사전이 존재하는지 확인합니다.
   - `ch_PP-OCRv4_det_infer.onnx` (텍스트 영역 검출 모델)
   - `korean_PP-OCRv4_rec_infer.onnx` (텍스트 인식 모델)
   - `korean_dict.txt` (텍스트 인식용 단어 사전)
3. `src/main/resources/data/` 경로에 의약품 사전 데이터가 존재하는지 확인합니다.
   - `medicines.csv`
...
3. `src/main/resources/models/` 디렉터리에 ONNX 모델 및 사전 파일(`ch_PP-OCRv4_det_infer.onnx`, `korean_PP-OCRv4_rec_infer.onnx`, `korean_dict.txt`)을 추가합니다.
4. 프로젝트를 빌드하고 실행합니다:
   ```bash
   ./gradlew bootRun
   ```